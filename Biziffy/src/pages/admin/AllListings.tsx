...
export const AllListings = () => {
  ...
  const fetchFullListings = async () => {
    ...
    setError(null);
    try {
      const res = await axios.get(`http://localhost:5000/api/get-all-listings`);
  ...
  const handleBulkAction = async () => {
    ...
    try {
      await axios.post(`http://localhost:5000/api/listing-bulk-action`, {
  ...
  const handleUpdatePublishStatus = async (id: string, newStatus: string) => {
    try {
      await axios.post(`http://localhost:5000/api/change-publish-status/${id}`, { status: newStatus });
      setFullListings(
  ...
  const handleDeleteListing = async (id: string) => {
    ...
    try {
      const response = await axios.get(`http://localhost:5000/api/delete-business-listing/${id}`);
